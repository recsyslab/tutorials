...
1. 対象の仮想マシンを選択し、**設定**ボタンをクリックする。
   1. **設定**ダイアログで下記を設定する。
      ...
      - **一般 > 高度 > ドラッグ&ドロップ**: `双方向`
      - **システム > マザーボード > メインメモリー**: （設定可能な範囲で最大メモリを推奨）
...
1. 作成したスナップショットを選択し、**クローン**ボタンをクリックする。
   1. 下記を設定する。
      - **名前**: `Linux Mint 22.3 MATE 64-bit_clean`
      - **パス**: `X:\VirtualBox VMs`
      - **すべてをクローン**: `選択`
...
1. **仮想マシン > 追加**を開く。
2. 下記ファイルを開く。
   - `X:\VirtualBox VMs\Linux Mint 22.3 MATE 64-bit\Linux Mint 22.3 MATE 64-bit.vbox`
3. 下記の仮想マシンが追加されたことを確認する。
   - `Linux Mint 22.3 MATE 64-bit`

### 「E_INVALIDARG」エラーで開けない場合
- VirtualBoxのバージョンが元のPCのものと一致しているか確認する。
- `*.vbox`ファイルを開き、UUIDが衝突していないことを確認する。
- `*.vbox`ファイルを開き、`<DVDImages>...</DVDImages>`の箇所を削除してみる。
