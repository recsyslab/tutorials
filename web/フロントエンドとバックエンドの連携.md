...
`api/hello/`ディレクトリに`urls.py`を作成し、下記コードを記述する。
...
...（略）...
Migrations for 'hello_db':
  api/hello_db/migrations/0001_initial.py
    - Create model Hello
(rsl-web) $ python manage.py migrate --settings config.settings.development
```

PostgreSQLの`app`データベースに`hello`テーブルが作成されていれば成功。
...
--------+----------------------------+-------+-------
...（略）...
 public | hello                      | table | rsl
(11 rows)
app=# INSERT INTO hello(world) VALUES ('123');
```

### フロントエンドからモデルの参照

`config/urls.py`に下記コードを追加する。
...
urlpatterns = [
    ...
    path('api/hello_db/', include('api.hello_db.urls')),    # 追加
]
```

`api/hello_db/`ディレクトリに`urls.py`を作成し、下記コードを記述する。

`~/dev/app/backend/api/hello_db/ursl.py`
```py
from django.urls import path
from . import views
...
`api/hello_db/views.py`を下記のように編集する。

`~/dev/app/backend/api/hello_db/views.py`
```py
from rest_framework.response import Response
from rest_framework.views import APIView
...
ブラウザで http://localhost:8000/api/hello_db/backend/ にアクセスし、モデルの値が取得できれば成功。

### フロントエンドとの連携

`frontend/app/`ディレクトリに`hello_backend_db`ディレクトリを作成する。
- `~/dev/app/frontend/app/hello_backend_db/`

作成した`hello_backend_db/`ディレクトリに`page.tsx`を作成し、下記コードを記述する。

`~/dev/app/frontend/app/hello_backend_db/page.tsx`
```tsx
'use client'
import axios from 'axios'
import { useEffect, useState } from 'react'
...
ブラウザで http://localhost:3000/hello_backend_db にアクセスし、「hello 123!」と表示されれば成功。
